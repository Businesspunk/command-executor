...
export class CommpressVideoCommand extends Command {
  ...
  protected async promptParameters(): Promise<void> {
    ...
    const width = await this.promptService.input<number>("Width", PromptType.Number, 1920);
    const height = await this.promptService.input<number>("Height", PromptType.Number, 1080);
